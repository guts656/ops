...
function inferResourceType(action, target, source) {
    if (source === 'host')
        return '主机';
    if (action.includes('账号') || action.includes('密码'))
        return '账号';
    if (action.includes('自愈'))
        return '自愈规则';
    if (action.includes('初始化'))
        return '系统';
    return target || '系统';
}
function getGenericRequestSummary(log) {
    const parsed = parseJson(log.detail);
    if (parsed && typeof parsed === 'object') {
        const summary = parsed.summary ?? parsed.requestSummary;
        if (typeof summary === 'string')
            return summary;
    }
    return log.detail;
}
function toGenericLog(log) {
    return {
        id: log.id,
        source: 'system',
...
function verifyHostHash(log, index, rows) {
    const chainInput = rows.map(({ previousHash: _previousHash, hash: _hash, createdAt: _createdAt, ...item }) => item);
    const expected = buildHostAuditHashChain(chainInput);
    return expected[index]?.previousHash === log.previousHash && expected[index]?.hash === log.hash;
}
function toHostLog(log, hashChainStatus) {
    return {
        id: log.id,
        source: 'host',
        time: log.time,
...
function matchesFilters(log, filters) {
    if (filters.startTime && log.timestamp < filters.startTime.getTime())
        return false;
    if (filters.endTime && log.timestamp > filters.endTime.getTime())
        return false;
    if (!matchesText(log.operator, filters.operator))
        return false;
    if (filters.actionType && log.actionType !== filters.actionType)
        return false;
    if (filters.resourceType && log.resourceType !== filters.resourceType)
        return false;
    if (!matchesText(log.clientIp, filters.clientIp))
        return false;
    return true;
}
async function getAllAuditLogs() {
    const [genericRows, hostRows] = await Promise.all([
        ...
        prisma.hostAuditLog.findMany({ orderBy: { createdAt: 'asc' } }),
    ]);
...
export async function getAuditLogDetail(id) {
    ...
    if (generic) {
        ...
        const requestBody = parsed && typeof parsed === 'object' ? parsed.request ?? parsed : generic.detail;
        const responseBody = parsed && typeof parsed === 'object' ? parsed.response ?? { result: generic.result } : { result: generic.result };
        return {
    ...
    const hostRows = await prisma.hostAuditLog.findMany({ orderBy: { createdAt: 'asc' } });
...
function exportCsv(logs) {
    ...
    return [headers.join(','), ...logs.map((log) => headers.map((key) => escapeCsvValue(log[key])).join(','))].join('\n');
}
...
export function getAuditExportTask(id) {
