...
const permissionSchema = z.custom<Permission>((value) => Object.values(PERMISSIONS).includes(value as Permission), '未知权限')
const roleSchema = z.enum(['admin', 'sre', 'auditor', 'viewer']) as z.ZodType<Role>
...
router.patch('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    if (paramId(req.params.id) === req.user!.id && !values.enabled) return res.status(400).json({ message: '不能禁用当前登录账号' })
...
router.patch('/:id/enabled', async (req: AuthRequest, res, next) => {
  try {
